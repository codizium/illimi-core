...
import type { FeedASTNode, FeedElementNode, FeedSlotNode, FeedTemplateBlockNode, FeedSlotPlaceholderNode } from '../ast/nodes.js';
...
export function mergeSlots(
  ...
  for (const node of layoutNodes) {
    if (node.type === 'Slot') {
      const slotNode = node as unknown as FeedSlotNode;
 ...
 */
export function extractSlots(nodes: FeedASTNode[]): SlotContentMap {
  const slots = new Map<string, FeedASTNode[]>();
  
  for (const node of nodes) {
    if (node.type === 'TemplateBlock') {
      const templateNode = node as FeedTemplateBlockNode;
      slots.set(templateNode.name, templateNode.children);
    }
    
    if (node.type === 'SlotPlaceholder') {
      const slotNode = node as FeedSlotPlaceholderNode;
